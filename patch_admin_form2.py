...
if start_idx != -1 and end_idx != -1:
    ...
    old_company_div = '<div>            <label className="block text-kite-text-light mb-1 text-[11px] uppercase">Company Name</label>            <input type="text" className="w-full border border-kite-border-soft rounded-sm p-1.5 bg-transparent" value={editingIpo.companyName || \'\'} onChange={e => setEditingIpo({...editingIpo, companyName: e.target.value})} />          </div>'
    new_company_div = '<div><label className="block text-kite-text-light mb-1 text-[11px] uppercase">Company Name</label><input type="text" list="admin-businesses-list" className="w-full border border-kite-border-soft rounded-sm p-1.5 bg-transparent" value={editingIpo.companyName || \'\'} onChange={e => setEditingIpo({...editingIpo, companyName: e.target.value.toUpperCase()})} placeholder="Search business..." /><datalist id="admin-businesses-list">{state.businesses.map(b => (<option key={b.id} value={b.shortName?.toUpperCase() || b.name.toUpperCase()} />))}</datalist></div>'
    
    admin_func = admin_func.replace(old_company_div, new_company_div)
    
    # Try slightly fuzzier replace if it didn't work
    if old_company_div not in admin_func:
        # regex to the rescue
        admin_func = re.sub(
    ...
    with open('src/pages/Bids.tsx', 'w') as f:
        f.write(new_content)
    print("Admin form updated via Python 2.")
else:
