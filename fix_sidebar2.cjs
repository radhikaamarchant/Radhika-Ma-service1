...
content = content.replace(/const LiveSidebarValue = React.memo\(\(\{[^]*?\}\);\n\}\);/m, `const LiveSidebarValue = React.memo(({ name, originalPrice, baseAmount, isOpen }: { name: string; originalPrice: number; baseAmount: number; isOpen: boolean }) => {
