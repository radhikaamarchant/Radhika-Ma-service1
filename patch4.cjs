...
// 1. Add orderTab state
content = content.replace(
  /const \[orderMode, setOrderMode\] = useState<"BUY" \| "SELL">("BUY");/,
  `const [orderMode, setOrderMode] = useState<"BUY" | "SELL">("BUY");\n  const [orderTab, setOrderTab] = useState<"REGULAR" | "CAP">("REGULAR");`
);

// 2. Fix the desktop view layout: Header, Tabs, Content, Bottom buttons
const desktopCodeEndRegex = /\s*\) : \(\s*<motion\.div\s*className="w-full max-w-\[680px\].*?<\/motion\.div>\s*\)\}\s*<\/motion\.div>\s*\)\}\s*<\/AnimatePresence>\s*\);\s*\}/s;

const newDesktopCode = `
          ) : (
            <motion.div
            ...
            >
              <div className={\`h-[70px] px-6 flex items-center justify-between transition-colors duration-300 \${orderMode === "BUY" ? "bg-[#4184F3]" : "bg-[#FF5722]"}\`}>
                ...
                <div 
                  className="flex bg-black/20 rounded-[4px] p-0.5 relative items-center cursor-pointer w-[120px] h-[32px]" 
                  onClick={() => { 
                    setOrderMode(orderMode === "BUY" ? "SELL" : "BUY"); 
                    setFormData({ ...formData, investorIds: [] }); 
                  }}
                >
                  <div className={\`absolute top-0.5 bottom-0.5 w-[58px] bg-white rounded-[3px] transition-transform duration-300 shadow-sm \${orderMode === "BUY" ? "translate-x-0" : "translate-x-[58px]"}\`} />
                  <span className={\`w-1/2 text-center text-[12px] font-semibold z-10 transition-colors \${orderMode === "BUY" ? "text-[#4184F3]" : "text-white/90"}\`}>BUY</span>
                  <span className={\`w-1/2 text-center text-[12px] font-semibold z-10 transition-colors \${orderMode === "SELL" ? "text-[#FF5722]" : "text-white/90"}\`}>SELL</span>
                </div>
              </div>
                ...
                {orderTab === "REGULAR" ? (
                  ...
                  <div className="grid grid-cols-2 gap-6">
                    <div className="space-y-1">
                      <label className="text-[10px] text-gray-500 dark:text-[#8F8F8F] uppercase tracking-wider">Duration (Months)</label>
                      <input
                        type="number"
                    ...
                    <div className="space-y-1">
                      <label className="text-[10px] text-gray-500 dark:text-[#8F8F8F] uppercase tracking-wider flex justify-between">
...
content = content.replace(desktopCodeEndRegex, newDesktopCode);
