...
code = code.replace(
  /<div key=\{b\.id\} onClick=\{\(\) => \{\s*if \(onNavigate\) \{\s*sessionStorage\.setItem\("mobileAddInvestmentBusinessId", b\.id\);\s*window\.dispatchEvent\(new Event\("mobileNavigateToInvestments"\)\);\s*onNavigate\("investments"\);\s*\}\s*\}\} className="p-4/g,
