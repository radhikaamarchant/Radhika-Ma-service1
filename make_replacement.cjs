const fs = require('fs');
let target = fs.readFileSync('target_chunk.txt', 'utf-8');

let replacement = `            </div>

            {/* Business & Investor Select (Fixed) */}
            <div className="bg-white dark:bg-transparent relative shrink-0 z-20">
            ...
            <div className="flex-1 overflow-y-auto bg-white dark:bg-[#1E2938]" style={{ paddingBottom: "100px" }}>
                {/* Main Inputs */}
                <div className="bg-white dark:bg-transparent relative">
                  
                  {/* Amount and Quantity */}`;

fs.writeFileSync('replacement_chunk.txt', replacement);
console.log("Done");
