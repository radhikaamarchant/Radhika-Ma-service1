...
export default function MyPnL() {
 ...
 { label: 'Commission Rate', value: `${inv.adminCommissionInvestorPct}%` }
    ...
    if (inv.status === 'completed' && inv.payoutDetails) {
      ...
      if (inv.payoutDetails.rmasMarketCover && inv.payoutDetails.rmasMarketCover > 0) {
        ...
        statement.push({
          ...
          details: [
            { label: 'Business Name', value: bName },
            { label: 'Completion Date', value: new Date(inv.payoutDetails.payoutDate).toLocaleDateString('en-IN') },
            { label: 'Cover Amount', value: formatINR(inv.payoutDetails.rmasMarketCover) },
            { label: 'Reason', value: 'Deducted from RMAS Profit to cover market loss' }
          ]
        });
      }
      
      if (inv.payoutDetails.rmasSubsidyPays && inv.payoutDetails.rmasSubsidyPays > 0) {
        ...
        statement.push({
          ...
          details: [
            { label: 'Business Name', value: bName },
            { label: 'Completion Date', value: new Date(inv.payoutDetails.payoutDate).toLocaleDateString('en-IN') },
            { label: 'Subsidy Amount', value: formatINR(inv.payoutDetails.rmasSubsidyPays) },
            { label: 'Reason', value: 'Deducted from RMAS Profit for subsidy payout' }
          ]
        });
      }
 ...
 <button onClick={() => setShowStatement(false)} className="p-2 rounded-full hover:bg-kite-bg text-kite-text-light transition-colors">
 <ArrowLeft className="w-4 h-4 md:w-6 md:h-6" />
 </button>
 <div>
 <h2 className="text-xs md:text-base font-medium text-kite-text tracking-tight">P&L Detailed Statement</h2>
 <p className="text-sm text-kite-text-light mt-1">Transaction history of commissions and taxes collected.</p>
 </div>
 </div>
 <div className="flex flex-wrap items-center gap-3">
 <div className="flex items-center space-x-2 bg-white border border-kite-border rounded-sm px-3 py-2">
 ...
 className="bg-black text-white px-4 py-2 text-sm rounded-sm flex items-center space-x-2 hover:bg-gray-800 transition-colors"
 >
 ...
 <div id="pnl-statement-table" className="bg-white border border-kite-border rounded-sm overflow-hidden">
 <div className="hidden print:block p-2 md:p-4 mb-4 md:mb-8">
 <h2 className="text-xs md:text-base font-medium text-kite-text tracking-tight border-b-2 border-black pb-2 mb-4">RMAS P&L Statement</h2>
 <p className="text-sm text-kite-text-light mb-2">Generated on: {new Date().toLocaleDateString('en-IN')} {new Date().toLocaleTimeString('en-IN')}</p>
 {(fromDate || toDate) && (
 <p className="text-sm text-kite-text-light mb-4 whitespace-nowrap">Period: {fromDate ? new Date(fromDate).toLocaleDateString('en-IN') : 'Start'} to {toDate ? new Date(toDate).toLocaleDateString('en-IN') : 'Present'}</p>
 )}
 </div>
 <div className="hidden md:block overflow-x-auto w-full max-w-full">
 <table className="w-full text-left text-sm min-w-[800px]">
 <thead className="bg-kite-bg border-b border-kite-border uppercase text-[10px] tracking-wider text-kite-text-light">
 <tr>
 ...
 <tr>
 <td colSpan={6} className="py-12 text-center text-kite-text-light uppercase tracking-widest text-xs">No Records Found for the selected dates.</td>
 </tr>
 ) : (
 filteredStatement.map((row) => (
 <tr key={`pnl_desk_${row.id}`} className="hover:bg-kite-bg transition-colors group">
 <td className="py-4 px-6 text-kite-text-light whitespace-nowrap">{new Date(row.date).toLocaleDateString('en-IN')}</td>
 <td className="py-4 px-6 text-center">
 <span className={`px-2 py-1 text-[10px] uppercase tracking-widest rounded-sm ${row.type === 'commission' ? 'bg-kite-blue/20 text-blue-800' : 'bg-blue-100 text-blue-800'}`}>
 {row.type}
 </span>
 </td>
 <td className="py-4 px-6 text-kite-text font-medium">{row.title}</td>
 <td className="py-4 px-6 text-kite-text-light">{row.source}</td>
 ...
 <button onClick={() => setSelectedBill(row)}
 className="px-3 py-1 bg-white border border-kite-border text-xs rounded-sm hover:bg-kite-bg transition-colors text-kite-text inline-flex items-center space-x-1"
 >
 ...
 {filteredStatement.length === 0 ? (
 <div className="py-12 text-center text-kite-text-light uppercase tracking-widest text-xs">No Records Found.</div>
 ) : (
 filteredStatement.map((row) => (
 <div key={`pnl_mob_${row.id}`} className="p-2 md:p-4 bg-white">
 <div className="flex justify-between items-start mb-2">
 <span className="font-medium text-kite-text">{row.title}</span>
 <span className={`px-2 py-1 text-[10px] uppercase tracking-widest rounded-sm ${row.type === 'commission' ? 'bg-kite-blue/20 text-blue-800' : 'bg-blue-100 text-blue-800'}`}>
 {row.type}
 </span>
 </div>
 <div className="text-xs text-kite-text-light mb-3 flex items-center justify-between">
 <span>{new Date(row.date).toLocaleDateString('en-IN')}</span>
 <span className="font-medium text-kite-text-light">From: {row.source}</span>
 </div>
 <div className="flex items-center justify-between mt-3 pt-3 border-t border-kite-border">
 <div className="font-medium text-kite-text text-xs md:text-base">
 {formatINR(row.amount)}
 </div>
 <button onClick={() => setSelectedBill(row)}
 className="px-3 py-1.5 bg-kite-bg border border-kite-border text-xs font-medium rounded-sm hover:bg-kite-bg transition-colors text-kite-text inline-flex items-center space-x-1"
 >
 <FileText className="w-3 md:w-3.5 h-3 md:h-3.5" />
 <span>View Bill</span>
 </button>
 ...
 <h3 className="text-xs md:text-base tracking-tight text-kite-text flex items-center">
 RADHIKA MA<span className="text-kite-blue ml-1">SERVICE</span>
 </h3>
 <div className="flex items-center space-x-3">
 <button onClick={handlePrintBill} className="px-3 py-1.5 text-xs bg-black text-white hover:bg-gray-800 rounded-sm font-medium transition-colors flex items-center space-x-1">
 <Download className="w-3 md:w-3.5 h-3 md:h-3.5" />
 <span>Download Bill</span>
 </button>
 <button onClick={() => setSelectedBill(null)} className="text-kite-text-light hover:text-kite-text transition-colors">
 ...
 <p className="text-xs uppercase tracking-widest text-kite-text-light mb-1">Receipt for</p>
 <p className="text-xs md:text-base font-medium text-kite-text">{selectedBill.title}</p>
 <div className="absolute top-0 right-0 opacity-10">
 <FileText className="w-10 h-10 md:w-16 md:h-16" />
 </div>
 </div>

 <div className="space-y-4 text-sm border-b border-dashed border-kite-border pb-6">
 {selectedBill.details.map((detail, idx) => (
 <div key={idx} className="flex justify-between items-start">
 <span className="text-kite-text-light">{detail.label}</span>
 <span className="text-kite-text text-right font-medium max-w-[60%] break-words leading-tight">{detail.value}</span>
 </div>
 ))}
 ...
 <span className="text-kite-text text-right font-medium">{new Date(selectedBill.date).toLocaleDateString('en-IN')}</span>
 </div>
 <div className="flex justify-between items-start">
 <span className="text-kite-text-light">Transaction ID</span>
 <span className="text-kite-text text-right text-xs bg-kite-bg font-mono px-2 py-0.5 rounded">{selectedBill.id.slice(-8).toUpperCase()}</span>
 </div>
 </div>

 <div className="flex justify-between items-center pt-2">
 <span className="text-xs md:text-base text-kite-text font-medium">Total Billed</span>
 <span className="text-xs md:text-base text-kite-text font-medium">{formatINR(selectedBill.amount)}</span>
 </div>
 </div>

 <div className="bg-kite-bg p-2 md:p-4 text-center border-t border-kite-border">
 <p className="text-xs text-kite-text-light">This is an auto-generated system receipt. All taxes and commissions are properly recorded according to RMAS policies.</p>
 </div>
 </div>
 </div>
 </div>
 ...
 <div>
 <h2 className="text-xs md:text-base font-medium text-kite-text tracking-tight">MY P&L Overview</h2>
 <p className="text-sm text-kite-text-light mt-2">Track total administrative commissions and tax deductions collected across all investments.</p>
 </div>

 <div className="grid grid-cols-1 md:grid-cols-2 gap-2 md:gap-4">
 <div className="bg-white rounded-sm border border-kite-border p-2 md:p-4 md:p-4">
 <div className="flex items-center justify-between gap-2 md:p-4 mb-3 md:mb-6">
 <span className="uppercase tracking-widest text-xs text-kite-blue">RMAS Profit Balance</span>
 ...
 <div className="bg-white rounded-sm border border-kite-border p-2 md:p-4 md:p-4">
 <div className="flex items-center justify-between gap-2 md:p-4 mb-3 md:mb-6">
 <span className="uppercase tracking-widest text-xs text-blue-600">HPG ITC TAX</span>
 <Landmark  className="w-4 h-4 md:w-6 md:h-6 text-blue-200" />
 </div>
 <div>
 <p className="text-xs md:text-base tracking-tight text-kite-text mb-2">{formatINR(totalTax)}</p>
 <p className="text-[10px] text-kite-text-light font-medium uppercase tracking-widest">Revenue from Tax Deductions</p>
 </div>
 </div>
 </div>
