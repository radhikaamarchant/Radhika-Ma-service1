...
export function getUnifiedBankBalance(
  ...
  investments.forEach(inv => {
    if (isAdmin) {
      ...
      if (biz && biz.id !== 'admin_business' && biz.registrationFee) balance -= biz.registrationFee;
      if (invt && invt.id !== 'admin_investor' && invt.rmasServiceCharge) balance -= invt.rmasServiceCharge;

      if (biz && inv.businessId === biz.id) {
        balance += inv.amount;
        if (inv.status === 'completed' && inv.payoutDetails) {
          const businessBurden = inv.payoutDetails.totalCredited + (inv.payoutDetails.rmasCommission || 0) + (inv.payoutDetails.happyIncomeTax || 0) + (inv.payoutDetails.rmasPrematurePenalty || 0) - (inv.payoutDetails.rmasSubsidyPays || 0);
