...
// 1. Insert totalProfitPay calculation
const totalFundedStr = `  const totalFunded = activeBusinessInvestments.reduce(
    (sum, inv) => sum + inv.amount,
    0,
  );`;
const newTotalFundedStr = `  const totalFunded = activeBusinessInvestments.reduce(
    (sum, inv) => sum + inv.amount,
    0,
  );

  const totalProfitPay = activeBusinessInvestments.reduce(
    (sum, inv) => sum + (inv.amount * (business.interestRate || 0)) / 100,
    0,
  );`;

if (content.includes(totalFundedStr)) {
  content = content.replace(totalFundedStr, newTotalFundedStr);
}

// 2. Insert Total Profit Pay view block
const totalFundedViewStr = `          <div className="bg-white dark:bg-kite-surface mb-2 py-4 px-5 border-b border-kite-border-soft flex justify-between items-center">
...
const newTotalFundedViewStr = `          <div className="bg-white dark:bg-kite-surface mb-2 py-4 px-5 border-b border-kite-border-soft flex justify-between items-center">
             <p className="text-[13px] md:text-[14px] text-kite-text font-normal">Total funded</p>
             <p className="text-[18px] md:text-[20px] font-normal text-kite-text">{formatINR(totalFunded).replace("₹", "")}</p>
          </div>
...
if (content.includes(totalFundedViewStr)) {
  content = content.replace(totalFundedViewStr, newTotalFundedViewStr);
}

// 3. Update the Current investor section
const currentInvestorOld = `             <h3 className="px-5 py-3 text-[14px] font-normal text-kite-text-light border-b border-kite-border-soft uppercase tracking-wider">Current investor</h3>
...
const currentInvestorNew = `             <h3 className="px-5 py-3 text-[14px] font-normal text-kite-text-light border-b border-kite-border-soft uppercase tracking-wider">Current investors</h3>
...
if (content.includes(currentInvestorOld)) {
  content = content.replace(currentInvestorOld, currentInvestorNew);
  console.log("Patched current investor section");
}
